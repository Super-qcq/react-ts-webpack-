 ...
 * ==========================================
 * 作用：直接用浏览器打开 dist/ 打包产物，/api/* 代理到 JSONPlaceholder
 * 效果等同于线上 Cloudflare Pages 的 _worker.js
 *
 * 用法：
 *   node src/student_system/serve.js
...
const PORT = 8081;
const DIST = join(__dirname, '..', '..', 'dist', 'student-system');
...
createServer((req, res) => {
  ...
  // ====== /api/* → 代理到 jsonplaceholder ======
  if (req.url.startsWith('/api')) {
    const target = 'https://jsonplaceholder.typicode.com' + req.url.slice(4);
  ...
  console.log(`\n  📦 静态目录: ${DIST}`);
  console.log(`  🔗 代理: /api/* -> https://jsonplaceholder.typicode.com`);
